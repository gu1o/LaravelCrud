...
@section('content')
    <div class="container">
        <h1 class="my-4">Edit Conflict</h1>
        <!-- Formulário de Cadastro -->


        <form action="{{ route('conflicts.update', $conflict->id) }}" method="POST" class="max-w-md mx-auto">
            @csrf
            @method('PUT')
            <div class="relative z-0 w-full mb-5 group">
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="name" id="name"
                        ...
                        placeholder=" " value="{{ $conflict->name }}" required />
                    <label for="name"
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="start_date" id="start_date"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " value="{{$conflict->start_date }}" required />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="end_date" id="end_date"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " value="{{ $conflict->end_date }}" required />
                    <label for="end_date"
